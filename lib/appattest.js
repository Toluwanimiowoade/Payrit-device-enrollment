...
function tmpPath(suffix) {
  return path.join(os.tmpdir(), "payrit-appattest-" + crypto.randomBytes(8).toString("hex") + suffix);
}
...
function issueCredCert(input) {
  ...
  try {
    ...
    fs.writeFileSync(rootKeyFile, input.rootPrivatePem, { mode: 0o600 });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        APPLE_NONCE_OID + " = DER:" + input.extensionDer.toString("hex").toUpperCase(),
        ""
      ].join("\n")
    );
    ...
    execFileSync(
      ...
      [
        "x509", "-req", "-in", csrFile,
        "-CA", rootCertFile, "-CAkey", rootKeyFile, "-set_serial", "0x" + crypto.randomBytes(8).toString("hex"),
        "-days", "30",
    ...
    return fs.readFileSync(leafFile, "utf8");
  } finally {
...
function buildAttestation(input) {
  ...
  var credCertDer = Buffer.from(pemToDerB64(credCertPem), "base64");
