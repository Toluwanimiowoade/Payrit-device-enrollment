...
function readRuntime() {
  try {
    var parsed = JSON.parse(fs.readFileSync(paths.runtimeFile(), "utf8"));
...
function writeRuntime(state) {
  ...
  fs.writeFileSync(paths.runtimeFile(), JSON.stringify(state, null, 2), { mode: 0o600 });
  return state;
}
