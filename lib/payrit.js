...
function requestAuthorization(key, body) {
  return call({
    method: "POST",
...
function getAuthorization(key, id) {
  return call({
    method: "GET",
    path: "/v1/authorizations/" + encodeURIComponent(id),
    label: "Look up pre-authorization",
    key: key
  });
}

function revokeAuthorization(key, id) {
  return call({
    method: "POST",
    path: "/v1/authorizations/" + encodeURIComponent(id) + "/revoke",
